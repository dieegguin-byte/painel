...
function allowedUsers(): Set<string> {
  return new Set((Deno.env.get("V4_SHADOW_ALLOWED_USER_IDS") ?? "").split(",").map((v) => v.trim()).filter(Boolean));
}
...
export default {
  fetch: withSupabase({ auth: "user", cors: "default" }, async (req, ctx) => {
    ...
    if (!allow.size) return Response.json({ error: "shadow_allowlist_not_configured" }, { status: 503 });

    const userId = String(ctx.userClaims?.id ?? "").trim();
    if (!userId || !allow.has(userId)) return Response.json({ error: "forbidden" }, { status: 403 });
